...
class Shape():
    ...
    def draw(self):
        ...
        for point in self.pointlist:
            ...
            #     ,self.width)
            points.append((self.startpoint.x + point.x * self.zoom, self.startpoint.y + point.y * self.zoom))
            oldpoint = point
...
class Ball():
    ...
    def __init__(self, screen, startpoint=Vec2d(5,5), move=Vec2d(1,0), radius = 15, color=(0,0,255), bossnumber=0, shape = "circle", linewidth=1, linelength=15, damage = 40):
        ...
        if self.shape == "circle":
            ...
            pygame.draw.line(self.surface, self.color, (radius, radius),
                     (radius+self.move.x*linelength, radius+self.move.y*linelength),linewidth)
...
class PygView(object):
    ...
    def paint(self):
        ...
        self.niklas_ship = Shape(self.screen, Vec2d(400, 200),
                                       (
                                        ...
                                        Vec2d(40, -10),
                                        Vec2d(-40, -40),
                                        Vec2d(0, 0)), cooldowntime = 15, color = (0, 200, 0))
        self.niklas_ship.rotate(90)
        self.niklas_ship.cooldown = 100 
    ...
    def run(self):
        ...
        while running:
            ...
            self.draw_text(text_player1, x=50, y=30, color=(200,0,0))
            self.draw_text(text_time, x = self.width//2-200, y=70, color=(100,0,100))
            self.draw_text(text_player2, x=self.width//2-200, y=30, color=(0,0,200))
            self.draw_text(text_player3, x=self.width-330, y=30, color=(0, 200, 0))
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    if self.niklas_ship.hitpoints > 0 and event.key == pygame.K_r:
                        if self.niklas_ship.cooldown > self.niklas_ship.cooldowntime:
                            ...
                            for k in range(90):
                                ...
                                Ball(self.screen, self.niklas_ship.startpoint+ move*-10, move*25,
                                    bossnumber=self.niklas_ship.number, color = bcolor, shape = "circle", radius=3, damage = 700)
            ...
            if d1.get_length() < c2.get_length():
                d = d1
            else:
                d = d2 
            d = d.normalized()
            ...
            #    pygame.draw.line (self.screen, (0, 200, 0), (self.niklas_ship.startpoint.x, self.niklas_ship.startpoint.y), (e.x, e.y), 12)                                                
            # --------- (auto)fire -------
            #c *= 0.05
            #d *= 0.05
            speedfactor = 0.05
